    ...
    with st.spinner("CAÇANDO OPORTUNIDADES..."):
        ...
        try:
            ...
            if not isinstance(res, list) or len(res) == 0:
                st.warning("📭 Sem dados encontrados para esta seleção.")
            else:
                arbs, monitoring = [], []
                for game in res:
                                ...
                                if market['key'] == 'totals':
                                    for o in market['outcomes']:
                                        ...
                                        if label not in totals_groups or o['price'] > totals_groups[label]['price']:
                                            totals_groups[label] = {"price": o['price'], "book": book['title'], "point": p, "side": o['name']}
                ...
                # --- EXIBIÇÃO: ENTRADA CONFIRMADA ---
                if arbs:
                    st.subheader("💰 ENTRADA CONFIRMADA (LUCRO IMEDIATO)")
                    for r in arbs:
                        lbl = f"✅ {r['data']['profit_pct']:.2f}% | {r['game']}"
                        if r['type'] == "TOTAL": lbl += f" (Linha: {r['line']})"
                ...
                # --- EXIBIÇÃO: RADAR NITRO (CORRIGIDO) ---
                st.subheader("👀 RADAR NITRO (VALOR PRÓXIMO)")
                if monitoring:
                    monitoring = sorted(monitoring, key=lambda x: x['data']['profit_pct'], reverse=True)
                    for r in monitoring:
                        lbl = f"📊 {r['data']['profit_pct']:.2f}% | {r['game']}"
                        if r['type'] == "TOTAL": lbl += f" (Linha: {r['line']})"
                        with st.expander(lbl):
                            st.write(f"**DÉFICIT/VALOR: R$ {r['data']['profit']:.2f}**")
                            cols = st.columns(len(r['names']))
                            for i, name in enumerate(r['names']):
                                info = r['details'][name]
                                cols[i].metric(f"R$ {r['data']['stakes'][i]:.2f}", f"{name} @ {info['price']}", info['book'])
                else:
                    st.info("Nenhuma oportunidade no radar.")
