...
st.title("🔥 NITRO ARB - RADAR GLOBAL V8.4")
...
# Unibet e William Hill REMOVIDAS conforme orientação do Maca
ALLOWED_BOOKS = "pinnacle,bet365,betano,1xbet,betfair_ex,marathonbet,888sport,bwin,stake"

# Filtros para Radar Total
MIN_PROFIT = -10.0
MAX_RESULTS = 20
        ...
        try:
            ...
            if opportunities:
                ...
                for arb in opportunities:
                    ...
                    header = f"{'✅ LUCRO' if p_pct > 0 else '📊 RADAR'}: {p_pct:.2f}% | {arb['game']}"
                    
                    with st.expander(header, expanded=(p_pct > 0)):
                        if p_pct > 0:
                        ...
                        for i, name in enumerate(arb["names"]):
                            ...
                            cols[i].write(f"**Apostar: R$ {arb['data']['stakes'][i]:.2f}**")
            else:
                st.info("Nenhuma odd encontrada para as casas ativas.")
